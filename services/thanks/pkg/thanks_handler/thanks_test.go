...
type mockThanksService struct {
	// このモックがどのような値を返すかを設定するためのフィールド
	GenerateThanksFunc func(ctx context.Context, name string) (string, error)
}

// GreetRepoインターフェースのメソッドを実装
func (m *mockThanksService) GenerateThanks(ctx context.Context, name string) (string, error) {
	// もし関数が設定されていれば、それを呼び出す
	if m.GenerateThanksFunc != nil {
		return m.GenerateThanksFunc(ctx, name)
	}
	// デフォルトの動作（何も設定されていない場合）
	return "", errors.New("GenerateThanksFunc is not set")
}
...
func Test_Greet(t *testing.T) {
	...
	testCases := []struct {
		name              string                                                 // テストケース名
		input             *thanksv1.ThanksRequest                                // 入力値
		mockGenerateGreet func(ctx context.Context, name string) (string, error) // モックの振る舞いを定義
		expected          *thanksv1.ThanksResponse                               // 期待されるメッセージ
		expectedErr       error                                                  // 期待されるエラー
	}{
		{
			name:  "正常系: 名前を渡した場合",
			input: &thanksv1.ThanksRequest{Name: "hori"},
			...
			expected: &thanksv1.ThanksResponse{
				Thanks: "Greet, hori! Thank you for using our service.",
			},
			expectedErr: nil,
		},
		{
			name:        "異常系: ハンドラでの入力値チェック",
			input:       &thanksv1.ThanksRequest{Name: ""},
			expected:    nil,
			expectedErr: connect.NewError(connect.CodeInvalidArgument, errors.New("name is required")),
		},
		{
			name:  "異常系: サービスがエラーを返す",
			input: &thanksv1.ThanksRequest{Name: "error-case"},
			mockGenerateGreet: func(ctx context.Context, name string) (string, error) {
				// 例えば internal/greet.ErrNotFound を返すケース
				return "", thanks.ErrNotFound // internal/greet からエラーをインポートする必要がある
			},
			expected: nil,
			// mapErrorToConnectCode で変換された後のエラーを期待値とする
			expectedErr: connect.NewError(connect.CodeNotFound, thanks.ErrNotFound),
		},
	}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			// Arrange (準備)
			s := &mockThanksService{GenerateThanksFunc: tc.mockGenerateGreet}
			h := Handler{service: s}

			// Act (実行)
			res, err := h.Thanks(context.Background(), tc.input)
